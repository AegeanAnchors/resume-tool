"""
doc_generator_therapy_new_grad.py

Builds the formatted Word document for the Therapy New Grad template
(PT, OT, SLP, PTA, COTA, RRT — recent graduates).
Uses Acute Formatted Example.docx as the base document.

Layout difference from standard Therapy:
  - CLINICAL EXPERIENCE comes FIRST (primary content for new grads)
  - EXPERIENCE section follows only if the candidate has paid work history
"""
...
def generate_document_therapy_new_grad(data: dict, output_path: str) -> str:
    ...
    # Clinical Experience first for new grads
    if clinical:
...
def _add_job_entry(doc, job):
    """
    Paid work experience entry:
      MM/YYYY – Present    FACILITY NAME, City, ST
                           Physical Therapy Aide (Staff)
