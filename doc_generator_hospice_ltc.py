"""
doc_generator_hospice_ltc.py

Builds the formatted Word document for the combined Hospice / LTC / SNF template.
Uses Acute Formatted Example.docx as the base document.

Job entry fields (all jobs):
  Employed Through, Discipline, Specialty, Role, Leadership/Charge, Additional Info

Additional fields shown only for Hospice jobs (is_hospice=True):
  EMR/Charting, Patient Caseload

Additional Info yellow prompt differs:
  - is_hospice=True  → "[ Recruiter: Please Verify Additional Hospice Info ]"
  - is_hospice=False → "[ Recruiter: Please Verify Additional LTC Info ]"
"""
...
YELLOW_ADDITIONAL_HOSPICE = "[ Recruiter: Please Verify Additional Hospice Info ]"
YELLOW_ADDITIONAL_LTC     = "[ Recruiter: Please Verify Additional LTC Info ]"
YELLOW_DATE               = "[ Date ]"
...
def generate_document_hospice_ltc(data: dict, output_path: str) -> str:
...
def _add_job_entry(doc, job):
    """
    Hospice / LTC job entry layout:
      MM/YYYY – Present    ORGANIZATION NAME, City, ST
                           Employed Through: [value]
                           Discipline: RN (Staff)
                           Specialty: [value]
                           Role: [value]
                           EMR/Charting: [value]       ← Hospice jobs only
                           Patient Caseload: [value]   ← Hospice jobs only
                           Leadership/Charge Experience: Yes  ← only if noted
                           Additional Info: [bullets or yellow]
                           • Bullet points
    """
    ...
    _add_hanging_line(doc, f"{dates}\t{org_line}" if dates else f"\t{org_line}")
    ...
    is_hospice = job.get("is_hospice", False)
    if is_hospice:
    ...
    if additional_bullets:
        ...
        yellow_msg = YELLOW_ADDITIONAL_HOSPICE if is_hospice else YELLOW_ADDITIONAL_LTC
        para = doc.add_paragraph(style="Position")
        _set_space_after(para, 0)
        label_run = para.add_run("Additional Info: ")
        _set_run_font(label_run, size=12, bold=False, italic=False)
        _add_yellow_run(para, yellow_msg)
