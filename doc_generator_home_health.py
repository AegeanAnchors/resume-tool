"""
doc_generator_home_health.py

Builds the formatted Word document for the Home Health (RN & LPN) template.
...
Additional fields shown only for Home Health jobs (is_home_health=True):
...
YELLOW_ADDITIONAL_INFO   = "[ Recruiter: Please Verify Additional Home Health Info ]"
YELLOW_DATE              = "[ Date ]"
...
def generate_document_home_health(data: dict, output_path: str) -> str:
...
def _add_job_entry(doc, job):
    """
    Home Health job entry layout:
      MM/YYYY – Present    ORGANIZATION NAME, City, ST
                           Employed Through: [value]
                           Discipline: RN (Staff)
                           Specialty: [value]
                           Role: [value]
                           EMR/Charting: [value]       ← HH jobs only
                           Patient Caseload: [value]   ← HH jobs only
    ...
    is_hh = job.get("is_home_health", True)
    if is_hh:
