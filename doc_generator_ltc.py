"""
doc_generator_ltc.py

Builds the formatted Word document for the Long Term Care (RN & LPN) template.

Mirrors Home Health / Hospice exactly — same base template, same layout:
  - "Organization" label
  - Role field
  - Additional Info field (always shown for LTC jobs — yellow prompt if not found)
  - No Bed Count, Trauma Level, or Teaching Status
  - Patient Caseload is per day/shift
  - Leadership/Charge appears ABOVE Additional Info
  - EMR/Charting and Patient Caseload only shown for is_ltc=True jobs
"""
...
YELLOW_ADDITIONAL_INFO = "[ Recruiter: Please Verify Additional LTC Info ]"
YELLOW_DATE            = "[ Date ]"


def generate_document_ltc(data: dict, output_path: str) -> str:
    """
    Builds the LTC formatted Word document.
...
def _add_education_section(doc, education):
    ...
    for edu in education:
        ...
        # Education dates use full month names — use 2.5" tab so all entries align
...
def _add_job_entry(doc, job):
    """
    Adds one complete LTC job entry.
    ...
    Layout:
      MM/YYYY – MM/YYYY    ORGANIZATION NAME, City, ST
                           ...
                           EMR/Charting: [value or yellow]     ← LTC jobs only
                           Patient Caseload: [value or yellow] ← LTC jobs only
    ...
    is_ltc = job.get("is_ltc", True)

    # ── EMR/Charting — only for LTC jobs ─────────────────────────────────
    if is_ltc:
        _add_field_line(doc, "EMR/Charting", job.get("emr"))

    # ── Patient Caseload — only for LTC jobs ─────────────────────────────
    if is_ltc:
    ...
    if additional_bullets:
        ...
        # LTC job with nothing found — always show yellow prompt
        para = doc.add_paragraph(style="Position")
        _set_space_after(para, 0)
        label_run = para.add_run("Additional Info: ")
        _set_run_font(label_run, size=12, bold=False, italic=False)
        _add_yellow_run(para, YELLOW_ADDITIONAL_INFO)
    # Non-LTC job with nothing found — skip Additional Info entirely
...
def _add_hanging_line(doc, text, tab_pos=2160):
    """
    Date at left margin, content after tab at `tab_pos` twips, wrapped lines
    also at `tab_pos`. Default 2160 (1.5") for MM/YYYY experience dates.
    Pass 3600 (2.5") for full month-name education dates.
    """
    para = doc.add_paragraph()
    _set_space_after(para, 0)
...
def _add_indented_line(doc, text, indent=2160):
    """All lines start at `indent` twips. Pass 3600 for education degree lines."""
    para = doc.add_paragraph()
    _set_space_after(para, 0)
...
def _add_education_line(doc, dates, school_line):
    """
    Education date + school name line with 2.5" hanging indent.
    Yellow [ Date ] placeholder if dates is missing.
    """
    para = doc.add_paragraph()
    _set_space_after(para, 0)
